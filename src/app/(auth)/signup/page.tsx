...
export default function SignupPage() {
  ...
  const handleEmailSignup = async (e: React.FormEvent) => {
    ...
    if (!name.trim() || !email.trim() || !password.trim()) {
      toast.error("Please fill in all display name, email, and password fields.")
      return
    }
    ...
    try {
      ...
      if (data?.session) {
        toast.success("Account created successfully! Welcome to CodeSense.")
        router.push("/dashboard")
        router.refresh()
      } else {
        ...
        <div className="w-full max-w-md space-y-6 z-10">
          <div className="text-center space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              Sign up to start reviewing your codebase files
            </p>
          </div>
            ...
            <form onSubmit={handleEmailSignup} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="name"
                  type="text"
                  placeholder="e.g. Prajakta Gochhayat"
                  value={name}
